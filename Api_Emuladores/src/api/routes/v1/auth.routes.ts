...
import { loginSchema, registerSchema, verifyOtpSchema, resendOtpSchema } from "../../dtos/auth.dto";
...
authRouter.post("/verify-otp", validateBody(verifyOtpSchema), (req, res, next) => {
  controller.verifyOtp(req, res).catch(next);
});

authRouter.post("/resend-otp", validateBody(resendOtpSchema), (req, res, next) => {
  controller.resendOtp(req, res).catch(next);
});
