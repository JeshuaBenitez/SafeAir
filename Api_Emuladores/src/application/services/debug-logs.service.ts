...
interface EmulatorState {
  ...
  devices: {
    minisplit: { isOn: boolean | null; targetTemperature: number | null };
    purifier: { isOn: boolean | null };
    extractor: { isOn: boolean | null };
  };
}
...
export function updateEmulatorState(
  ...
  devices?: { minisplit?: any; purifier?: any; extractor?: any }
): void {
  const existing = emulatorStates.get(emulatorId) || {
    ...
    devices: {
      minisplit: { isOn: null, targetTemperature: null },
      purifier: { isOn: null },
      extractor: { isOn: null },
    },
  };
  ...
  if (devices) {
    if (devices.minisplit) {
      existing.devices.minisplit = {
        isOn: devices.minisplit.isOn ?? existing.devices.minisplit.isOn,
        targetTemperature: devices.minisplit.targetTemperature ?? existing.devices.minisplit.targetTemperature,
      };
    }
    if (devices.purifier) {
      existing.devices.purifier.isOn = devices.purifier.isOn ?? existing.devices.purifier.isOn;
    }
    if (devices.extractor) {
      existing.devices.extractor.isOn = devices.extractor.isOn ?? existing.devices.extractor.isOn;
    }
  }
