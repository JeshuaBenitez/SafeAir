...
function Import-EnvLan {
  $envFile = Join-Path (Get-Location) ".env.lan"
...
if (-not $env:MQTT_HOST) { $env:MQTT_HOST = "100.79.106.54" }
...
if ($null -eq $env:SAFEAIR_EMULATOR_IDS) { $env:SAFEAIR_EMULATOR_IDS = "" }
if (-not $env:SAFEAIR_DYNAMIC_PROVISIONING_ONLY) { $env:SAFEAIR_DYNAMIC_PROVISIONING_ONLY = "true" }
...
Write-Host "  SAFEAIR_DYNAMIC_PROVISIONING_ONLY=$env:SAFEAIR_DYNAMIC_PROVISIONING_ONLY"
Write-Host "  SERVER_PORT=$env:SERVER_PORT"
if (-not $env:SAFEAIR_EMULATOR_IDS) {
  Write-Host "  Modo: provisionamiento dinamico"
  Write-Host "  Esperando configuracion desde API..."
} else {
  Write-Host "  Modo: IDs manuales desde SAFEAIR_EMULATOR_IDS"
}
